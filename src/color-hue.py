...
def process(filename, target_hue = 0, saturation_weight = 1):
    ...
    for pixel_r, pixel_g, pixel_b in zip(r.getdata(), g.getdata(), b.getdata()):
        ...
        # use weighted hue
        hue1 = (saturation_weight) * h + (1 - saturation_weight) * target_hue
        # use weighted saturation
        sat1 = (saturation_weight) * s + (1 - saturation_weight) * pink_sat
        # convert back to RGB color space
        rgb = colorsys.hsv_to_rgb(hue1, sat1, v)
